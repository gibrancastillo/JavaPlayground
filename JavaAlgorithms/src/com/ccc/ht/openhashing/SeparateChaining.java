...
public class SeparateChaining {
	...
	public void insert(Word wordToInsert) {
		...
		// Calculate the hash code or hash value for the [String] Word, this is the index in the array, no need to convert hash code to index
		int hashCode = stringHashFunction(wordToHash);
		
		System.out.println("[String] Key: " + wordToHash + " --> [int] hash code: " + hashCode + " <--> [int] array-based list index: " + hashCode);
		
		// Add the new word to the hash table [array] and set the key for the Word
		theHashTableArray[hashCode].insert(hashCode, wordToInsert);
	}
	
	public Word find(String wordToFind) {
		// Calculate the hash code for the word (key)
		int hashCode = stringHashFunction(wordToFind);
		
		// NEW
		Word theWord = theHashTableArray[hashCode].find(hashCode, wordToFind);
		
		System.out.println("[String] Key: " + wordToFind + " --> [int] hash code: " + hashCode + " <--> [int] array-based list index: " + hashCode);
	...
	public int stringHashFunction(String key) {
		...
		System.out.println("wordToHash or piece of input [key]: " + key);
		
		for(int i = 0; i < key.length(); i++) {
			// 'a' has the ASCII character code of 97 so subtract 96 to make our letters start at 1.
			int charCode = key.charAt(i) - 96;
			int hkVTemp = hashCode;
			
			// Calculate the hash key using the 26 letters in the alphabet a-z plus blank.
			hashCode = (hashCode * 27 + charCode) % this.arraySize;
			
			System.out.println("Hash Key Value " + hkVTemp + " * 27 + Character Code " + charCode + " % arraySize " + this.arraySize + " = " + hashCode);
		}
		
		// The Hash Value maps the input [a key] to a particular index in the hash table [an array].
		System.out.println("hashKeyValue or an integer commonly refer to as a hash value: " + hashCode);
